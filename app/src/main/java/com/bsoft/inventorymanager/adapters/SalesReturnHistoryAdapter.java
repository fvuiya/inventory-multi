...
public class SalesReturnHistoryAdapter extends RecyclerView.Adapter<SalesReturnHistoryAdapter.ViewHolder> {
    ...
    private final List<SaleReturn> saleReturns;
    private final SimpleDateFormat dateFormat = new SimpleDateFormat("MMM dd, yyyy", Locale.getDefault());

    public SalesReturnHistoryAdapter(List<SaleReturn> saleReturns) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        SaleReturn saleReturn = saleReturns.get(position);

        // Use document ID substring for visual ID if actual display ID is not available
        String displayId = saleReturn.getDocumentId();
        ...
        if (saleReturn.getReturnDate() != null) {
            holder.tvReturnDate.setText(dateFormat.format(saleReturn.getReturnDate().toDate()));
        } else {
            holder.tvReturnDate.setText("N/A");
        }

        holder.tvPartyName
                .setText(saleReturn.getCustomerName() != null ? saleReturn.getCustomerName() : "Unknown Customer");
        holder.tvReturnAmount.setText(String.format(Locale.getDefault(), "%.2f", saleReturn.getTotalRefundAmount()));
    }

    @Override
    public int getItemCount() {
        return saleReturns.size();
    }
